    ...
    return new (OrderCycle = (function() {
          ...
          return function(data) {
            if (data['success']) {
              ...
              if (destination != null) {
                ...
                return StatusMessage.display('success', 'Your order cycle has been updated.');
              }
            } else {
      ...
      OrderCycle.prototype.confirmNoDistributors = function() {
        if (this.order_cycle.outgoing_exchanges.length === 0) {
          return confirm('There are no distributors in this order cycle. This order cycle will not be visible to customers until you add one. Would you like to continue saving this order cycle?');
        } else {
          return true;
        }
      };
