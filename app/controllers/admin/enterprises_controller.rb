  ...
  class EnterprisesController < ResourceController
    ...
    def register
      if params[:sells] == 'unspecified'
        flash[:error] = "Vui lòng chọn một gói"
      ...
      if @enterprise.update_attributes(attributes)
        flash[:success] = "Xin chúc mừng! Thành lập #{@enterprise.name} đã hoàn thành!"
        redirect_to admin_path
      else
        flash[:error] = "Không thể hoàn thành thành lập #{@enterprise.name}"
    ...
    def bulk_update
      ...
      if @enterprise_set.save
        flash[:success] = "Doanh nghiệp được cập nhật thành công"
        ...
        flash[:action] = "Đã cập nhật #{pluralize(touched_enterprises.count, 'enterprise')}: #{touched_enterprises.map(&:name).join(', ')}"

        redirect_to main_app.admin_enterprises_path
      else
        @enterprise_set.collection.select! { |e| touched_enterprises.include? e }
        flash[:error] = 'Không thể cập nhật'
        render :index
      end
    end
